...
* [x] --muted: #f0ecff (밝게 완료)
* [x] --dim: #ddd6f5 (밝게 완료)
* [x] body font-size: 17px (상향 완료)
* [x] 섹션 설명 텍스트 font-size 16px

## 🔜 다음 작업 — 색깔 시스템 + 기승전결 (우선순위 순)

### 1. 색깔 시스템 정비 (가장 먼저)
현재 포트폴리오 전체가 보라색 단색 위주. 섹션 성격에 따라 색상에 의미를 부여해야 함.

**목표:**
- intro-banner → 보라색 glow (현재 유지, OK)
- 경력 섹션 → 보라색 (타임라인 기반, 현재 OK)
- MCP/코드/연동 증거 섹션 → **초록색 계열** (#10b981) 강조. "실제로 작동하는 것" 의미.
- TC 테이블 섹션 → 보라/중립 (현재 OK)
- 스킬 섹션 → 보라색 유지
- 연락 섹션 → 심플하게

**실행:**
- `.badge-live`, `.proof-card::before`, `.commit-stat-num` → 초록 강조 유지
- MCP 섹션 sec-head에 초록 badge 유지
- 섹션 배경 미묘한 차이로 구분감 강화: bg → bg2 → bg → bg3 → bg2 → bg 순환

### 2. 기승전결 흐름 강화
포트폴리오가 "이야기"처럼 읽혀야 함. 현재 섹션 간 연결이 약함.

**목표 내러티브:**
- **기(intro-banner + 경력)**: "나는 6.9년 현장 QA다. 이게 쌓인 맥락이다."
- **승(MCP 섹션)**: "그 경험으로 반복 작업을 자동화했다. 이게 어떻게 돌아가는지 보여주겠다."
- **전(코드+연동+TC)**: "말이 아니라 실제 코드, 실제 스크린샷, 실제 TC다."
- **결(스킬+연락)**: "이 사람이 할 수 있는 것들이고, 필요하면 연락해라."

**실행 방법:**
- 각 섹션 상단에 1-2줄 "브릿지 문장" 추가 (앞 섹션과 연결)
- MCP 섹션 → "6.9년 현장에서 반복된 작업들을 보고 만든 것" 문구 강화
- 코드 섹션 → "MCP가 만든 것들이 실제로 어떻게 생겼는지" 브릿지
- TC 섹션 → "이 TC들도 전부 MCP가 만들었다" 연결
...
1. index.html GitHub에서 가져오기
2. 색깔 시스템 CSS 변수 + 섹션별 적용
3. 섹션 간 브릿지 문장 추가
4. push_files로 index.html + Feedbackback.md 같이 push
...
* index.html은 단일 파일 101KB+. SHA 없이 push_files로 처리.
