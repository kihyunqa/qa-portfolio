...
## 🎯 STEP 95에서 변경한 내용 (index.html)

| 위치 | 변경 전 | 변경 후 |
|------|---------|---------|
| Hero 우측 stats 4번째 | `∞` / `확장 가능성` | `P1 2건` / `보안 버그 발견` |
| index.html SHA | `4c0794e...` | `af44f7f28b40df32b2e3bbfbc1212c94860c42dd` |

---

## 🎯 다음 Claude가 해야 할 포트폴리오 업그레이드 (우선순위 순)

### 🔴 우선순위 1 — 버그 스토리 섹션 index.html에 인라인 카드로 추가
현재 버그 스토리는 GitHub 링크로만 연결되어 있음. 임팩트가 가장 큰 개선.
docs/bug-stories.md 핵심 3건을 포트폴리오 본문에 카드 형식으로 인라인 표시.
- **BUG-001**: 보안 토큰 허점 (P1) — 로그아웃 후에도 토큰 재사용 가능
- **BUG-002**: 결제 1원 오차 에스컬레이션 (P1) — 소수점 반올림 오차
- **BUG-003**: 저속 네트워크 알림 중복 (P2) — 네트워크 시뮬레이션 테스트
- 섹션 번호: `09` 또는 기존 섹션 뒤에 삽입
- 카드 스타일: `proof-card` 스타일 참고 + 빨간 상단 라인 대신 버그 심각도별 컬러

### 🟡 우선순위 2 — TC 테이블 필터 버튼 추가
...
7. **index.html 현재 SHA**: `af44f7f28b40df32b2e3bbfbc1212c94860c42dd`
...
*최종 업데이트: 2026-04-06 · STEP 95 완료 · v26*
*Hero stat 4번째 ∞ → P1 2건 보안 버그 발견 실제 수치로 교체 완료 🎉*
