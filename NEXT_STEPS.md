...
| STEP 5 | POM helpers, signup/notification spec, portfolio-summary | 2026-03-30 |
| STEP 6 | testcase_payment/signup/cross-browser, skills 추가 | 2026-03-30 |
| STEP 7 | docs/ai-qa-vision, jira-mcp-plan, qa-onboarding, README 전면 업데이트 | 2026-03-30 |
| STEP 8 | testcase_regression 20건 보강, screenshots/PROOF.md | 2026-03-30 |
| STEP 9 | mcp-portfolio.spec.js 강화, cover-letter.md | 2026-03-30 |
| STEP 13 | index.html 숫자 업데이트: spec 12개, TC 145건+, 파일 80개+ | 2026-03-31 |
| STEP 14 | docs/linkedin-post.md 수치 최신화 (145건+, 12 spec) ✅ 검증됨 | 2026-03-31 |
| STEP 14-B | docs/cover-letter.md 오타 수정 + 수치 반영 | 2026-03-31 |
| STEP 15 | docs/interview-qa.md 수치 최신화 (TC 30→145건+, spec 4→12개) | 2026-03-31 |
| STEP 16 | docs/interview-qa-advanced.md 오타 대량 수정 + 수치 최신화 | 2026-03-31 |
...
```
현재: screenshots/PROOF.md에 가이드만 있고 실제 PNG 없음

방법 A — 직접 캡처 후 채팅창에 이미지 업로드하면 Claude가 커밋:
  1. Slack 새-채널 알림 화면 스크린샷 캡처
  2. GitHub Actions qa-notify.yml 성공 화면 캡처
  3. Notion TC 페이지 캡처
  → 채팅창에 이미지 올리면 Claude가 screenshots/ 폴더에 커밋

방법 B — GitHub 웹에서 직접 업로드:
  https://github.com/kihyunqa/qa-portfolio/upload/main/screenshots

★ 이게 "실제로 됐다"를 증명하는 가장 강력한 포트폴리오 자료
```
...
```
모든 문서 수치 최신화 완료 → 지금 바로 공유 가능한 상태!

1. LinkedIn 게시: docs/linkedin-post.md 버전 1~4 중 선택
2. 이력서에 링크 추가: https://kihyunqa.github.io/qa-portfolio
3. 커버레터: docs/cover-letter.md 활용
4. 면접 준비:
   - 기본: docs/interview-qa.md (수치 최신화 완료 ✅)
   - 심화: docs/interview-qa-advanced.md (오타 수정 + 수치 최신화 완료 ✅)
```

### STEP 17 — docs/ 나머지 파일 수치 점검 [선택]
...
필요시: get_file_contents로 읽어서 오래된 수치 있으면 수정
우선순위 낮음 — STEP 10, 11, 12 먼저 진행 권장
```
...
2. **index.html은 58KB — MCP API 한도 초과 가능** → 수정 필요 시 GitHub 웹 Ctrl+H 사용
3. **숫자 부풀리기 금지** — 실제 파일 수 확인 후 기재
4. **새 파일 무분별 생성 금지** — 기존 파일 업데이트 우선
5. **NEXT_STEPS.md 업데이트 필수** — 스텝 완료 후 SHA 확인 후 업데이트
6. **토큰 효율**: 할 수 있는 양만 처리하고 나머지는 인수인계
...
# 2단계: 다음 스텝 파일 확인
# 현재 다음 스텝은 STEP 10 (스크린샷) 또는 STEP 11 (Jira MCP)
# 사용자가 스크린샷 이미지를 올려주면 → STEP 10 처리
# 안 올려주면 → STEP 17 (나머지 docs 파일 수치 점검) 진행 가능

# 3단계: 작업 완료 후 반드시 NEXT_STEPS.md 업데이트
```
...
*최종 업데이트: 2026-03-31 · STEP 16 완료 · 다음: STEP 10 (스크린샷 PNG 추가) / STEP 11 (Jira MCP) / STEP 17 (docs 나머지 점검)*
